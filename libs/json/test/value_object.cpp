...
BOOST_AUTO_TEST_CASE(all)
{
   ...
   value["number"] = 78.3;
   BOOST_CHECK_EQUAL(value["number"]  , json::value{ 78.3 });
   ...
   BOOST_CHECK_EQUAL(value["foo"]["size"]  , json::value{ 93 });
   BOOST_CHECK_EQUAL(value["foo"]["color"] , json::value{ "black" });

   value["foo"]["color"] = "blue";
   BOOST_CHECK_NE   (value["foo"]["color"] , json::value{ "black" });
   BOOST_CHECK_EQUAL(value["foo"]["color"] , json::value{ "blue" });
   ...
   BOOST_CHECK_EQUAL(iter->value() , json::value{ "blue" });
   ++iter;
   BOOST_CHECK_EQUAL(iter->name()  , "size");
   BOOST_CHECK_EQUAL(iter->value() , json::value{ 93 });
   ++iter;
   ...
   BOOST_CHECK_EQUAL(value["bar"][2], json::value{ 1234.5 });

   value.set("cafe", "open");
   BOOST_CHECK_EQUAL(value["cafe"], json::value{ "open" });
