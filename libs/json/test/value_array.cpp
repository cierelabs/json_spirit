...
BOOST_AUTO_TEST_CASE(basic_test)
{
   ...
   BOOST_CHECK_EQUAL(v[1], json::value{ "foo" });
   v[1] = "gorp";
   BOOST_CHECK_EQUAL(v[1], json::value{ "gorp" });
   BOOST_CHECK_EQUAL(v.at(1), json::value{ "gorp" });
   v.at(1) = "bar";
   BOOST_CHECK_EQUAL(v.at(1), json::value{ "bar" });
   ...
   BOOST_CHECK_EQUAL(v[8][1], json::value{ 1 });

   v[8][1] = true;
   BOOST_CHECK_EQUAL(v[8][1], json::value{ true });
   BOOST_CHECK_NE   (v[8][1], json::value{ false });
...
BOOST_AUTO_TEST_CASE(reference_test)
{
   ...
   BOOST_CHECK_EQUAL(v[4][2], json::value{ 2 });

   v[3][2] = "gorp";
   BOOST_CHECK_EQUAL(v[3][2], json::value{ "gorp" });
   BOOST_CHECK_NE(v[4][2], json::value{ "gorp" });
   BOOST_CHECK_EQUAL(v[4][2], json::value{ 2 });
...
BOOST_AUTO_TEST_CASE(iterator_test)
{
   ...
   while(iter != iter_end)
   {
      BOOST_CHECK_EQUAL(*iter, json::value{ count++ });
      *iter = count;
      ++iter;
   }
